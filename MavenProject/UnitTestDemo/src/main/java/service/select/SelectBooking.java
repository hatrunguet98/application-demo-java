...
public class SelectBooking implements ISelect<Booking> {
    ...
    @Override
    public Booking getById(String id, ArrayList<Booking> listT) {
        Booking booking = new Booking();
        for (int i = 0; i < listT.size(); i++) {
            if (listT.get(i).getId().equalsIgnoreCase(id)) {
                booking = listT.get(i);
                break;
            }
        }
    ...
    public void delete(String id) {
        ...
        for (int i = 0; i < listBookingsFile.size(); i++) {
            if (listBookingsFile.get(i).getId().equalsIgnoreCase(id)) {
    ...
    public void update(Booking booking) {
        listBookingsFile = new BookingFile().get();
        for (int i = 0; i < listBookingsFile.size(); i++) {
            if (listBookingsFile.get(i).getId().equalsIgnoreCase(booking.getId())) {
                listBookingsFile.remove(i);
                listBookingsFile.add(booking);
                break;
            }
