...
//    }
    ...
    public void updateSaleItem(SaleItem saleItem) {
        ...
        System.out.printf("\n");
        System.out.printf("Sale item's type : ");
        String typeConsole = scanner.nextLine();
        String type;
        if (typeConsole.isEmpty()) type = saleItem.getType();
        else type = typeConsole;
        System.out.printf("\n");
        System.out.printf("Sale item's numberOfPerson : ");
        String nuPerConsole = scanner.nextLine();
        int numberOfPerson;
        if (nuPerConsole.isEmpty()) numberOfPerson = saleItem.getNumberOfPerson();
        else numberOfPerson = Integer.valueOf(nuPerConsole);
        System.out.printf("\n");
        System.out.printf("Sale item's descride : ");
        String descrideConsole = scanner.nextLine();
        String descride;
        if (descrideConsole.isEmpty()) descride = saleItem.getDescribe();
        else descride = descrideConsole;
        System.out.printf("\n");
        System.out.printf("Sale item's totalPrice : ");
        String total = scanner.nextLine();
        double totalPrice;
        if (total.isEmpty()) totalPrice = saleItem.getUnitPrice();
        else totalPrice = Double.valueOf(total);
        System.out.printf("\n");
        System.out.printf("Sale item's acreageIn(m2) : ");
        String acreageIn = scanner.nextLine();
        double acreage;
        if (acreageIn.isEmpty()) acreage = saleItem.getAcreage();
        else acreage = Double.valueOf(total);
        System.out.printf("\n");
        System.out.printf("Sale item's rateType : ");
        System.out.printf("\n");
        System.out.printf(" Street View : 1");
        ...
        if (rateType.equals("2")) rate = 1.2;
        if (rateType.equals("1")) rate = 1.5;
        if(rateType.equals("1")) rate = saleItem.getRateType();
        else rate = 1;
        System.out.printf("Sale item's address : ");
        String address = scanner.nextLine();
        if(address.isEmpty()) address=saleItem.getAddress();
        System.out.printf("\n");
        new SelectSaleItem().update(new SaleItem.SaleItemBuilder(saleItem.getId(), type, rate, address, hostId, numberOfPerson, totalPrice).setUpdateAudit(date).setDescribe(descride).setAcreage(acreage).setCreatedAudit(saleItem.getCreatedAudit()).build());
    }
}
